...
FAT32_File::FAT32_File(char Fname, char type){
	this->Fname=Fname;
	this->type=type;
}

char FAT32_File::getFname(){return this->Fname;}
char FAT32_File::getType(){return this->type;}
void FAT32_File::setFname(char *FileName){this->Fname=FileName;}
void FAT32_File::setType(char *FileType){this->type=FileType;}
...
void FAT32_File::Report(){
	Check();
	USART_Send("\r\n Name of File is: ");
	USART_Send(fileInstance.Fname);
	USART_Send(" | Type of File is: ");
	USART_Send(fileInstance.type);
	USART_Send(" | FCA: ");
...
uint8_t FAT32_File::Append_Cluster_RootDir(){
	File temp;
	uint16_t clust= (RETURNING_NEXT_OPEN_CLUST());
	UpdateFatWithNewClustLoc(temp,clust,1);
	Update_FATWithNewEndingTAG(clust);
	return 0;
}
